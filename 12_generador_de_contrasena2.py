import random
import string

def  password_generator():
   """
   It returns one or several passwords
   based on the numbers the user inputs
   when asked to do so
   """
   charactersY = string.ascii_letters + string.digits + string.punctuation
   
   while 1:
      lenghtX = int(input("What legth do you want your password to be?: "))
      total_of_pasword = int(input("How many password you need?: "))
      print("\n")
      
      for x in range(0,total_of_pasword):
         password=""
         for i in range(0,lenghtX):
            password_charactersX=random.choice(charactersY)
            password = password + password_charactersX
            
         print("Here is your password: "+password)
         print("\n")
      
      
      break

def run():
   caller = password_generator()

   
            
if __name__=='__main__':
   run()